...
class ExpenseTracker:
    ...
    def validate_input(self, amount_str, category, date_str):
        """Проверка корректности ввода с подробными сообщениями об ошибках"""
        # Проверка суммы
        if not amount_str:
            messagebox.showerror("Ошибка", "Поле суммы не может быть пустым")
            return False
        ...
        try:
            ...
            messagebox.showerror("Ошибка", "Некорректная сумма. Введите числовое значение")
            return False
        ...
        # Проверка даты
        if not date_str:
            messagebox.showerror("Ошибка", "Поле даты не может быть пустым")
            return False

        try:
            datetime.strptime(date_str, "%Y-%m-%d")
        except ValueError:
            messagebox.showerror("Ошибка", "Некорректный формат даты. Используйте формат ГГГГ-ММ-ДД")
            return False
    ...
    def add_expense(self):
        ...
        if not self.validate_input(amount_str, category, date_str):
            return
